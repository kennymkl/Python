...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                distances[j] = self.get_euclidean_distance(data.iloc[:, self.start_var:self.end_var], self.centroids.iloc[j, :])
            ...
            max_distance_index = distances.min(axis=1).idxmax()
            
            # Append the selected data point to the set of centroids.
            next_point = data.iloc[max_distance_index, self.start_var:self.end_var]
            next_point = next_point.set_axis(self.centroids.columns).to_frame().T
            self.centroids = pd.concat([self.centroids, next_point], ignore_index=True)
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if isinstance(point1, pd.Series) and isinstance(point2, pd.Series):
            ...
            distance = np.sqrt(((point2 - point1) ** 2).sum(axis=1))
            return distance.astype(np.float64)

        else:
            raise ValueError("Invalid combination of data types for point1 and point2.")
    ...
    def group_observations(self, data):
        ...
        for i in range(self.k):
            ...
            # Hint: Use the get_euclidean_distance() function that we have
            # defined in this class.
            distances[i] = self.get_euclidean_distance(data.iloc[:, self.start_var:self.end_var], self.centroids.iloc[i, :])
    ...
    def train(self, data, iters):
        ...
        while i < iters and not flag_groups and not flag_centroids:
            ...
            flag_centroids = cur_centroids.equals(centroids)
