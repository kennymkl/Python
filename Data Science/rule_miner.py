...
class RuleMiner(object):
    ...
    def get_support(self, data, itemset):
        ...
        # Update the mask to be True only for rows where all items in the itemset are present
        for item in itemset:
            mask &= data[item] == 1
        
        # Calculate the support by summing up the True values in the mask
        support = mask.sum()
    ...
    def get_confidence(self, data, rule):
        ...
        support_antecedent = self.get_support(data, antecedent)

        # Combine antecedent and consequent to form the full itemset
        full_itemset = antecedent + consequent

        # Calculate the support of the full itemset
        support_full_itemset = self.get_support(data, full_itemset)

        # Calculate confidence as the ratio of the support of the full itemset to the support of the antecedent
        if support_antecedent > 0:
            confidence = support_full_itemset / support_antecedent
        else:
            confidence = 0
    ...
    def get_association_rules(self, data):
        ...
        for rule in rules:
            ...
            if confidence >= self.confidence_t:
                association_rules.append({'rule': rule, 'confidence': confidence})
